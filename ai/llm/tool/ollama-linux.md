...
## 配置
    ...
    payload = {
        ...
        "think": False, # 关闭深度思考
        "options": {
            "num_ctx": 20960, # 最多上下文 20960
            "num_predict": 8000, # 最多生成 8000 tokens
        },
    }
...
- `prompt_eval_count` → 模型对 prompt 分词后评估的 token 数
- `eval_count` → 模型生成的 token 数
- `eval_duration` / `prompt_eval_duration` → 推理消耗时间
- `load_duration` → 加载模型时间
...
> 注意, ollama 并发会改掉所有 ollama 模型的显存占用
>
> 所以需要并发的模型, 请使用 vllm 或者 sglang

+ 编辑 ` sudo sublime /etc/systemd/system/ollama.service  `

  注意, 需要放在 [Service] 中, [Install] 之前
...
## Ollama Dify 集成 Embed Model

Ollama context size: `ollama show xxxmodel`

Dify-Ollama-Context-Size: Model->Context Size: 默认4096
...
+ 一分钟定位
  ...
  ```
  用 1 分钟定位到底是哪一种（在“手机热点 USB”状态下跑）
  ...
  # 2) 看 registry 域名解析到了哪些地址（同时看 v4/v6）
  getent ahosts registry.ollama.ai
  getent ahosts registry-1.docker.io   # 如果你说的“拉镜像”是 docker
  
  # 3) 分别强制 IPv4 / IPv6 访问测试（很关键）
  curl -4Iv https://registry.ollama.ai/v2/
  curl -6Iv https://registry.ollama.ai/v2/
  
  # 4) 粗测 MTU（1472 对应 1500 MTU；失败就很像 MTU/PMTU 问题）
...
+ 常见处理办法
  ...
  把它 MTU 调到 1400（如果你还套了代理/VPN，建议 1380 更稳）：
